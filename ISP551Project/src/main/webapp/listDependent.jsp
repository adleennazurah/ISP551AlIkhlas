...
<br><br>
	<table border="1">
		...
		<c:forEach items="${dependents}" var="d" varStatus="dependents">
		<tr>
			<td><c:out value="${d.dependentid}"/></td>
			<td><c:out value="${d.dependentname}"/></td>
			<td>0<c:out value="${d.telephoneno}"/></td>
			<td><a class="btn btn-primary" href="ViewDependentController?id=<c:out value="${d.dependentid}"/>">View</a></td> 
            <td><a class="btn btn-primary" href="UpdateDependentController?id=<c:out value="${d.dependentid}"/>">Update</a></td>
            <td><button class="btn btn-danger" id="<c:out value="${d.dependentid}"/>" onclick="confirmation(this.id)">Delete</button></td>
		</tr>
		</c:forEach>
	</table>
	<script>
		function confirmation(id)
		{
			...
			if(r == true)
			{
				location.href = 'DeleteDependentController?id=' + id;
